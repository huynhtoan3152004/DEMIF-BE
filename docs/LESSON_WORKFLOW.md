...
- `Lesson.level` và `Lesson.lessonType` hiện lưu dưới dạng string trong DB và response, ví dụ `Beginner`, `Intermediate`, `Advanced`, `Expert`, `Dictation`, `Shadowing`.
- FE nên gửi string cho các field này từ bây giờ; backend vẫn nhận được payload số cũ trong giai đoạn chuyển tiếp và tự normalize về giá trị chuẩn.
- Migration mới đã convert dữ liệu cũ trong bảng `Lessons` từ int sang text theo mapping cũ, nên dữ liệu hiện hữu không bị mất.
...
Payload request cho FE:

- `level`: string, ví dụ `Beginner` hoặc `Expert`.
- `lessonType`: string, ví dụ `Dictation` hoặc `Shadowing`.
- Backend vẫn chấp nhận giá trị số cũ trong thời gian chuyển tiếp, nhưng FE nên gọi string để tránh lệch mapping.

Request mẫu cho FE:

```json
{
	"youTubeUrl": "https://www.youtube.com/watch?v=pJY0mBWHPw4",
	"captionLanguage": "en",
	...
	"isPremiumOnly": false,
	"displayOrder": 0,
	"tags": "youtube,transcript,bbc",
	"status": "draft",
	"titleOverride": "Cassie School Starts Later",
...
```json
{
	"lessonId": "2d0d1f7a-0fd7-4fd8-9a7b-0e7fdb1f2c11",
	"title": "Cassie School Starts Later",
	"videoId": "pJY0mBWHPw4",
	"embedUrl": "https://www.youtube.com/embed/pJY0mBWHPw4",
	"durationSeconds": 157,
	"hasCaptions": true,
	"captionLanguage": "en",
	"isAutoGeneratedCaptions": false,
	"captionSegmentCount": 42,
	"transcriptWordCount": 612,
	"hasDictationTemplates": true,
	"status": "draft",
	"message": "✅ Lesson đã tạo thành công với Dictation templates cho 4 levels!"
}
```
...
- Các field `level` và `lessonType` trong request này cũng dùng string như trên; backend normalize numeric legacy input nhưng không khuyến khích FE tiếp tục gửi int.
...
{
	"title": "Cassie School Starts Later",
	"description": "Lesson import từ Tactiq transcript",
	...
	"thumbnailUrl": "https://img.youtube.com/vi/pJY0mBWHPw4/hqdefault.jpg"
}
```
...
```json
{
	...
	"durationSeconds": 157,
	"hasDictationTemplates": true,
	"message": "✅ Tạo bài 'Cassie School Starts Later' thành công: 42 segments, 612 từ. Đã generate DictationTemplates cho 4 levels. Status = draft → PATCH /status = published khi sẵn sàng."
}
```
...
Response lesson summary/detail cũng trả `level` và `lessonType` dưới dạng string để FE render trực tiếp mà không cần map enum.

Ví dụ response detail:

```json
{
	"id": "2d0d1f7a-0fd7-4fd8-9a7b-0e7fdb1f2c11",
	"title": "Cassie School Starts Later",
	"description": "Lesson import từ Tactiq transcript",
	"lessonType": "Dictation",
	"level": "Expert",
	"category": "academic",
	"mediaUrl": "https://www.youtube.com/embed/pJY0mBWHPw4",
	"audioUrl": "https://www.youtube.com/embed/pJY0mBWHPw4",
	"mediaType": "youtube",
	"durationSeconds": 157,
	"thumbnailUrl": "https://img.youtube.com/vi/pJY0mBWHPw4/hqdefault.jpg",
	"fullTranscript": "9:00 in the morning and Cassie is still in bed...",
	"hasDictationExercise": true,
	"availableDictationLevels": ["Beginner", "Intermediate", "Advanced", "Expert"],
	"isPremiumOnly": false,
	"completionsCount": 0,
	"avgScore": 0,
	"tags": "youtube,transcript,teenagers",
